...
def get_transitions(tagged_sents, states, transition_probability, emission_probability):
    ...
    for state, dictionary in transition_probability.items():
        ...
        for n_state in dictionary:
            transition_probability[state][n_state] = (
                transition_probability[state][n_state] / total_words
            )

    for state, dictionary in emission_probability.items():
        total_words = sum(emission_probability[state].values())
        for word in dictionary:
            emission_probability[state][word] = (
                emission_probability[state][word] / total_words
            )
...
def assign_unk(tok):  # noqa: PLR0911
...
def get_word_tag(line, vocab):  # noqa: D103
    if not line.split():
        word = "--n--"
        tag = "--s--"
        return word, tag
...
def preprocess(vocab, data_fp):
    ...
    with open(data_fp, "r") as data_file:
        ...
        for _cnt, word in enumerate(data_file):
    ...
    assert len(orig) == len(open(data_fp, "r").readlines())  # noqa: S101
    assert len(prep) == len(open(data_fp, "r").readlines())  # noqa: S101
...
def predict_pos(prep, y, test_y, test_x, freqs, tagged_counts):  # noqa: PLR0913
    """
    Input:
